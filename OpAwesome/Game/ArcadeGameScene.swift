...
//                        fakedeathButton?.fillColor = .green
    ...
    func createEnemies() {
        ...
        for _ in 0..<8 {
            ...
            enemy.zPosition = 3
            enemy.run(SKAction.repeatForever(SKAction.animate(with: enemyCharacter.animations[Orientation.Bottom]!, timePerFrame: 0.1)), withKey: ActionKeys.animation.rawValue)
            ...
            let moveAction = SKAction.run {
                if self.decideMove() {
                    ...
                    let orientation = self.enemyCharacter.updatePlayerOrientation(angle: CGFloat(angle))
